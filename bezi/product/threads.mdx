...
<Note>
  **For optimal response quality, keep threads short and focused on a single topic or task**.

  When prompting in a thread with long or multi-task prompt history, Bezi is more likely to accidentally reference the wrong information when processing the response.
</Note>
