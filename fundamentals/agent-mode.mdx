...
Agent Mode is Bezi's _actionable_ setting. It implements changes directly in the engine editor, automatically.

Bezi has complete access to the engine editors. For the most part, if you as a user can do something in the Editor, Bezi is able to do it as well. 

Bezi does not use an MCP to connect and implement changes in the editor. MCP's standardized method delivers unreliable results, has inefficient token use, and requires ongoing maintenance by the user.

Bezi's Editor integration is customized per engine to provide more accurate, reliable results.

---

## Using Agent Mode

Select the **mode dropdown** in the prompt textbox and select **Agent**. 

Agent Mode covers direct editor implementation for the most part if you as a user can do something in the Editor, Bezi has a chance to be able to do it as well
...
There are pre-built Actions (below) and [Custom Actions](/fundamentals/custom-actions) for everything not provided in Bezi.
...
#### GameObject **Modification and Scene Layout**
...
#### **Component Functionality & Dynamic Behavior**
