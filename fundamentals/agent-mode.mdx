...
Bezi does not use an MCP to connect and implement changes in the editor. MCP's one-size-fits-all method has unreliable results, inefficient token use, and ongoing maintenance by the user. The Editor integration is customized per engine to provide more accurate, reliable results.
