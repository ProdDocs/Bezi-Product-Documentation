...
These are the Workspace sources you give Bezi access to read from and write to. **All active Connections (engine projects, local folders) are available as read-only context for threads in the same Workspace.** Connections are added and managed from Workspace settings.
