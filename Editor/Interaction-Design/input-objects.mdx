...
<AccordionGroup>
  <Accordion title="Attaching Input objects to actions " defaultOpen={false}>
    ...
    3. Set the desired action(s) for the Input object

    By default, there are three platforms (Desktop, Gamepad, XR) available for each Input object.&#x20;

    Toggle between the platforms by selecting the names and use the dropdown to assign the Input object for each action.
